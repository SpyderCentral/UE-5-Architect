...
import { Play, Zap, Box, FunctionSquare, Workflow, Braces, Music, Grid } from 'lucide-react';
...
const BlueprintNode: React.FC<BlueprintNodeProps> = ({ data, connectedPins, onNodeDown }) => {
  ...
  return (
    <div 
      ...
      className="absolute flex flex-col w-[280px] rounded-[12px] overflow-hidden shadow-[8px_16px_30px_rgba(0,0,0,0.7)] font-sans select-none z-10 group ring-1 ring-white/5 hover:ring-white/20 transition-shadow cursor-default"
      style={{ 
        left: data.x, 
        top: data.y,
    ...
    >
      <div className={`px-3 py-2 flex items-center gap-2.5 border-b border-white/10 ${getHeaderStyle()}`}>
         <div className="flex-shrink-0"><HeaderIcon /></div>
         <span className="text-[13px] font-bold text-white drop-shadow-md truncate w-full tracking-wide antialiased">{data.name}</span>
