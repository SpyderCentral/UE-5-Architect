...
import { Folder, FileCode, CheckSquare, Settings, Cpu, Variable, ChevronDown, ChevronRight, Zap, ArrowRightCircle, Loader2, ShoppingBag, ExternalLink, Tag, ListOrdered, Braces, Workflow, SlidersHorizontal, MousePointer2, Box, FunctionSquare, ShieldCheck, Activity, Link2 } from 'lucide-react';
...
const TaskCard: React.FC<TaskCardProps> = ({ task, onNavigateToBlueprint }) => {
  ...
  return (
    ...
    >
      {isExpanded && <div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.8)]"></div>}

      <div 
        onClick={() => setIsExpanded(!isExpanded)}
        className="p-6 cursor-pointer flex items-start gap-5 relative z-10"
      >
        ...
        <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start gap-4">
                <h4 className={`text-lg font-bold transition-colors ${isExpanded ? 'text-blue-100' : 'text-slate-200 group-hover:text-white'}`}>
                    {task.title}
                </h4>
            ...
            <p className="text-slate-400 text-sm mt-2 leading-relaxed line-clamp-2 group-hover:text-slate-300 transition-colors font-light">
                {task.description}
            </p>
